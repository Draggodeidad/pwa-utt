...
export function useInspectionListState({ records, hasActiveFilters, isLoading = false, error = null }: UseInspectionListStateInput) {
  // Keep the server and first browser render identical; read navigator only after mounting.
  const [connectivity, setConnectivity] = useState<"online" | "offline" | "unstable">("online");
