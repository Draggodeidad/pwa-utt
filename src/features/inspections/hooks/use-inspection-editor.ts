...
export function useInspectionEditor(initial: InspectionEditorValues) {
  ...
  const save = async () => { if (!validate()) return false; setState("saving"); await new Promise<void>((resolve) => window.setTimeout(resolve, 350)); const offline = !navigator.onLine; dirtyRef.current = false; setValues((current) => ({ ...current, syncStatus: offline ? "pending" : "synced" })); setState(offline ? "offline-saved" : "saved"); setToast(true); return true; };
  ...
  return { values, state, errors, toast, mutate, validate, save, addFinding, updateFinding, removeFinding, setState };
}
